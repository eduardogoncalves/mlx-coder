...
/// Verifies `RemoteAPIError.isContextOverflow` / `.reportedContextWindow` — the
...
final class RemoteAPIContextOverflowTests: XCTestCase {
    ...
    func testDetectsLlamaCppExceedContextSizeError() {
        ...
        let error = RemoteAPIError.http(provider: "rtx5060ti", status: 400, body: body)
    ...
    func testReportedPromptTokensNilWhenAbsent() {
        ...
        let error = RemoteAPIError.http(provider: "OpenRouter", status: 400, body: body)
    ...
    func testIgnoresNon400Errors() {
        ...
        let error = RemoteAPIError.http(provider: "OpenRouter", status: 500, body: body)
        XCTAssertFalse(error.isContextOverflow)
    }

    func testIgnoresNonHTTPErrorCases() {
        XCTAssertFalse(RemoteAPIError.notConfigured.isContextOverflow)
        XCTAssertFalse(RemoteAPIError.transport(provider: "OpenRouter", detail: "reset").isContextOverflow)
        XCTAssertFalse(RemoteAPIError.decoding(provider: "OpenRouter", detail: "bad json").isContextOverflow)
    }
}
