...
public struct TaskTool: Tool {
    ...
    static func baseInstructions(for profileName: String, responseMode: String = "summary") -> String? {
        ...
        case .securityReview:
            return "You are the SECURITY_REVIEW agent. Inspect the code for REAL, pointable security risks: input validation, command/path/SQL injection, secret and data leakage, authorization-boundary gaps, unsafe deserialization, unsafe defaults. Use `code_graph_explore` to check a risky function's blast radius (who calls it, from where) before judging severity — an unguarded call from an authenticated-only path is a different risk than one reachable from user input. Do NOT edit files. Report each issue as `file:line — risk — concrete fix`, ordered by severity. Only flag a risk you can point to in the code; skip speculative or purely stylistic concerns. If you find nothing exploitable, say so explicitly. \(outputRules)"
        case .docs:
    ...
    static func defaultTools(for profileName: String) -> [String] {
        ...
        case .testEngineering:
            return ["read_file", "read_many", "list_dir", "glob", "grep", "code_search", "bash", "build_check"]
        case .securityReview:
            return ["read_file", "read_many", "list_dir", "glob", "grep", "code_search", "code_graph_explore"]
        case .docs:
            return ["read_file", "read_many", "list_dir", "glob", "grep", "code_search", "write_file", "edit_file"]
        case .planner:
